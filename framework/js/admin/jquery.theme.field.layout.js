(function ($) {
	...
	function PeFieldLayout(target, conf) {
		...
		function addBlock(type,where,fun,animate) {
			...
			if (isNew && module.container) {
				...
				if (firstContainer.find("> li").length === 0) {
					...
					msg += '<div class="pe-help-container pe-down"><div class="pe-help-message">Click to add more blocks to this %0</div><div class="pe-arrow-down"></div></div>'.format(module.conf.messages.type);
		...
		function autosave(event,jqxhr,settings) {
			if (settings.data.match(/action=autosave/)) {
